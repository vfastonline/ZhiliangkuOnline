...
from utils.polyv.video_api import get_video_msg
from utils.tools import time_str_to_second
		...
		try:
			...
			if request_type == "upload" and state:  # 已上传
				...
				if data_list:
					...
					duration = time_str_to_second(duration_str)
				...
				Video.objects.filter(video_id=ObjectId(state)).update(**update_dict)
