...
class Notification(BaseModelMixin):
	...
	user = models.ForeignKey(User, verbose_name="用户", related_name='NotificationUsers', db_index=True,
...
class UserNotificationsCount(BaseModelMixin):
	...
	unread = models.IntegerField("未读消息数", default=0)

	def __str__(self):
		return '<UserNotificationsCount %s: %s>' % (self.user.username, self.unread)
